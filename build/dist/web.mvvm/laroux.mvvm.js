/**
 * laroux.js - A jquery substitute for modern browsers (web.mvvm bundle)
 *
 * @version v2.2.0
...
var mvvm = {
    ...
    rebind: function rebind(appKey) {
        ...
        var callback = function callback(ev, elem) {
            var binding = mvvm.bindStringParser(elem.getAttribute('lr-event'));
        ...
        for (var i = 0, _length = app.eventElements.length; i < _length; i++) {
            dom.setEvent(app.eventElements[i].element, app.eventElements[i].binding[null], callback);
        }
    },
    ...
    scanElements: function scanElements(app, element) {
        for (var i = 0, atts = element.attributes, m = atts.length; i < m; i++) {
            if (atts[i].name === 'lr-bind') {
                var binding1 = mvvm.bindStringParser(atts[i].value);
                ...
                var binding2 = mvvm.bindStringParser(atts[i].value);
