...
let web_dom = {
    ...
    unsetEvent: function (element, eventname, callback) {
        ...
        for (let i1 = 0, length1 = elements.length; i1 < length1; i1++) {
            for (let i2 = 0, length2 = web_dom.eventHistory.length; i2 < length2; i2++) {
                let item = web_dom.eventHistory[i2];
    ...
    clone: function (element, type, container, target) {
        ...
        if (type !== undefined && type != web_dom.cloneReturn) {
            ...
            } else { // type == web_dom.cloneInsertBefore
